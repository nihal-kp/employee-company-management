...
class LoginController extends Controller
{
    ...
    */
    ...
    public function __construct()
    {
        ...
        $this->middleware('checkStatus:customer')->except('logout', 'otpVerify');
    }
    ...
    protected function authenticated(Request $request, $user)
    {
        ...
        if ($request->ajax()) {
            ...
            if ($user->otp_verification_status !== 1) {
                ...
                $message = 'Welcome to Medizone Online, your OTP for login is ' . $verification_otp;
    ...
    protected function validateLogin(Request $request)
    {
        $request->validate([
            ...
            $this->username().'required' => 'The email or phone number field is required.',
        ]);
    }
    ...
    protected function sendFailedLoginResponse(Request $request)
    {
        ...
        if (!Customer::where('email', $request->email)->orWhere('phone', $request->email)->exists()) {
            if(is_numeric($request->get('email'))) {
                $errors['email'] = trans('auth.phone_not_found');
            }
            else {
            ...
            if ($customer && (!Auth::guard('customer')->attempt(['email' => $request->email, 'password' => $request->password]) || !Auth::guard('customer')->attempt(['phone' => $request->email, 'password' => $request->password]))) {
    ...
    public function loginWithOtp(Request $request)
    {
        ...
        if (is_numeric($request->get('phone'))) {
            ...
            if ($customer) {
                ...
                $customer->email = request('phone') . '@medizone.com';
                $customer->verification_otp = $verification_otp = rand(1000, 9999);
                $customer->status = 1;
